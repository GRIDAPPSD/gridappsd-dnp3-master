...
class MyMaster:
    ...
    def __init__(self,
        ...
        if not self.stack_config:
            ...
            self.stack_config.master.responseTimeout = openpal.TimeDuration().Seconds(5)
        ...
        _log.debug('RemoteAddr=%s LocalAddr=%s', self.stack_config.link.RemoteAddr, self.stack_config.link.LocalAddr)
...
class SOEHandler(opendnp3.ISOEHandler):
    ...
    def __init__(self, name, device, dnp3_to_cim,gapps):
        ...
        self.gapps=gapps
        _log.debug('SOEHandler init for %s', name)
        _log.debug('dnp3_to_cim loaded: %s', dnp3_to_cim)
        super(SOEHandler, self).__init__()

    def get_msg(self):
        with self.lock:
            # return and clear so next publish gets only fresh updates
            msg = dict(self.CIM_msg)
            self.CIM_msg.clear()
            return msg

    def get_dnp3_msg_AI(self):
        with self.lock:
            _log.debug('self._dnp3_msg_AI length %d', len(self._dnp3_msg_AI))
    ...
    def update_cim_msg_binary_rtu(self, CIM_msg, index, value, conversion,model):
        ...
        CIM_units = conversion[index]['CIM units']
        CIM_attribute = conversion[index]['CIM attribute']
        _log.debug('binary phases %s %s', CIM_phases, CIM_units)
        _log.debug('binary units type %s %s', type(CIM_units), CIM_units)
        if CIM_units not in model:
    ...
    def update_cim_msg_binary(self, CIM_msg, index, value, conversion,model):
        ...
        if 'Binary input' in conversion and index in conversion['Binary input']:
            ...
            _log.debug('binary phases %s', CIM_phases)
            _log.debug('binary units type %s %s', type(CIM_units), CIM_units)
            if CIM_units not in model:
                _log.debug('%s not in model', CIM_units)
                return
            for CIM_phase in CIM_phases:
                ...
                _log.debug('binary value update %s', value)
    ...
    def Process(self, info, values):
        ...
        visitor = visitor_class()
        values.Foreach(visitor)
        _log.debug('Analog packet received with %d measurements', len(visitor.index_and_value))
        for index, value in visitor.index_and_value:
...
def command_callback(result=None):
    ...
    """
    _log.debug("Received command result with summary: %s", opendnp3.TaskCompletionToString(result.summary))
...
def restart_callback(result=opendnp3.RestartOperationResult()):
    if result.summary == opendnp3.TaskCompletion.SUCCESS:
        _log.debug("Restart success | Restart Time: %s", result.restartTime.GetMilliseconds())
    else:
        _log.debug("Restart fail | Failure: %s", opendnp3.TaskCompletionToString(result.summary))
